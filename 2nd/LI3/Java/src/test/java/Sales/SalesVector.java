...
public class SalesVector implements SalesInterface, Serializable {

    private static final long serialVersionUID = -5389528048317485677L;
    /**
    ...
    public List<Map.Entry<String, Long>> mostSoldProducts(int limit){
        ...
        /*return  sortedListByQuantity.entrySet().stream()
                .map(e -> new Query6Pair(e.getKey(),(int)e.getValue().stream().map(SaleInterface::getClientID).distinct().count()))
                .limit(limit)
                .collect(Collectors.toList());*/
    }
