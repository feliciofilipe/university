...
public class UMS implements IUMS {
  ...
  */
  ...
  public void addUser(Class clazz, String[] input, Class[] classes)
    ...
    if (!admins.containsKey(input[0])
      ...
      getDAO(clazz)
          .put(
              input[0],
              clazz.getConstructor(classes).newInstance(Tools.argsGenerator(input, classes)));
    else {
  ...
  public void makeEntryRequest(String id)
    ...
    this.getPallets(id)
        .values()
        .forEach(pallet -> this.entryRequests.put(pallet.getId(), (new EntryRequest(pallet, id))));
  }
      ...
      if (entryRequests.containsKey(e.getTruckID())) {
        entryRequests
            ...
            .put(
                e.getId(),
                new Pallet(
                    e.getId(), e.getProduct(), e.getQuantity(), e.getCompany(), e.getType()));
      } else {
        Map<String, Pallet> pallets = new HashMap<>();
        pallets.put(
            e.getId(),
            new Pallet(e.getId(), e.getProduct(), e.getQuantity(), e.getCompany(), e.getType()));
