...
public class StandardizedDAO<K, C> implements Map<K, C> {
  ...
  */
  public StandardizedDAO(String table, C clazz, List<String> columns) {
    try (Connection conn =
            DriverManager.getConnection("jdbc:mariadb://" + Config.DATABASE + Config.CREDENTIALS);
  ...
  public C get(Object o) {
    ...
    try {
      Connection conn =
          DriverManager.getConnection("jdbc:mariadb://" + Config.DATABASE + Config.CREDENTIALS);
      Statement stm = conn.createStatement();
      String sql = "SELECT * FROM " + this.table + " WHERE " + this.columns.get(0) + " =" + o + ";";
      ResultSet rs = stm.executeQuery(sql);
      if (rs.next()) {
  ...
  public C put(K key, C value) {
    ...
    Object[] methods2 =
        Arrays.stream(methods)
            .filter(y -> y.getName().startsWith("get") && !y.getName().startsWith("getClass"))
            .toArray();
    ;
    int i = 0;
    try {
