...
public class EntryRequestController extends UserController {
  ...
  public void addPalletButtonHandler(ActionEvent event) {
    ...
    if (!productField.getText().equals("")
      ...
      fxmlTableRowLoader.setLocation(getClass().getResource("../views/entryRequestTableRow.fxml"));
      try {
        ...
        String type = ((ToggleButton) typeGroup.getSelectedToggle()).getId().split("Button")[0];
        ((Button) fxmlTableRowLoader.getNamespace().get("deleteButton"))
            .setOnAction(
                e -> {
                  vbox1.getChildren().remove(hbox);
                  try {
                    wms.removeUserPallet(wms.getId(), this.indexMap.get(hbox));
                  } catch (NoSuchMethodException
                      | InvocationTargetException
                      | IllegalAccessException noSuchMethodException) {
                    PopUpController.popup(0, "ERROR", "SOMETHING IS WRONG WITH THIS REMOVAL");
                  }
