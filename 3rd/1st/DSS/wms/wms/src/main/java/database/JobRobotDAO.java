...
public class JobRobotDAO {
            ...
            ResultSet rs =
                    stm.executeQuery(
                            "SELECT IDJobRobot, SP.Station, JR.Station FROM "
                                    + "JobRobot AS JR INNER JOIN Pallet AS SP ON SP.Pallet=JR.Pallet ");
            if (rs.next()) {
    ...
    public Pair<Integer, Integer> getFirstPosition(String idJob) {
        ...
        try {
            ...
            ResultSet rs =
                    stm.executeQuery(
                            "SELECT S.X, S.Y FROM JobRobot AS JR INNER JOIN Station_Pallet AS SP ON SP.Pallet=JR.Pallet"
                                    + " INNER JOIN Station AS S ON S.IDStation=SP.Station\n"
                                    + "WHERE IDJobRobot = "
                                    + idJob
                                    + ";");
            if (rs.next()) {
